...
import { fetchBooks, fetchPage } from "../actions/action";
...
class BooksPage extends Component {
  ...
  componentDidUpdate(prevProps){
    ...
    if(prevProps.match.params.currentPage !== id) {
  ...
  render() {
    const { pending, books, booksCount, page, itemsPerPage, match } = this.props;
    const currentPage = match.params.currentPage ?? page;
      ...
      <>
        ...
        <Container>
          <Pagination
            ...
            renderItem={(item) => (
              item.page ? (
              <Link className="pagination-link" to={`/books/page/${item.page}`}>
